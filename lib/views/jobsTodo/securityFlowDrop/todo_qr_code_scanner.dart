...
class TodoDropQrScanner extends StatefulWidget {
  final AdvertiseModel advertiseModel;
  const TodoDropQrScanner({Key? key,required this.advertiseModel}) : super(key: key);

  @override
  State<TodoDropQrScanner> createState() => _TodoPickQrScannerState();
}

class _TodoPickQrScannerState extends State<TodoDropQrScanner> {
  ...
  void initState() {
    ...
    super.initState();
    context.read<JobsTodoProvider>().addSelected2(widget.advertiseModel.barCodes2!.toList());
  }
            ...
            child: Column(
              ...
              children: [
                ...
                GestureDetector(
                    onTap: () async{
                      try{
                        final qrCode = await FlutterBarcodeScanner.scanBarcode('#ff6666', 'Cancel', true,widget.advertiseModel.dropScanType=="Barcode"? ScanMode.BARCODE:
                        ScanMode.QR);
                      ...
                      ListView.builder(
                        ...
                        itemBuilder: (context,index){
                          return Row(
                            ...
                            children: [
                              myText(
                                text: obscureString(widget.advertiseModel.barCodes2![index].toString()),
                                fontFamily: "Poppins",fontWeight: FontWeight.w400, size: 20.sp, color: dashColor,
                              ),
                              GestureDetector(
                                onTap: (){
                                  context.read<JobsTodoProvider>().check2(index,widget.advertiseModel.barCodes2![index].toString(),result,context);
                                },
                                child: Container(
                                  padding:const EdgeInsets.all(10),
                                  decoration: BoxDecoration(
                                      color: context.watch<JobsTodoProvider>().selected2[index]==true? Colors.greenAccent:whiteColor,
                        ...
                        itemCount: widget.advertiseModel.barCodes2!.length,),
                      SizedBox(height: 10.h),
                    ],
                  ),
                ),
                ...
                MyFancyButton(
                  ...
                  if(context.read<JobsTodoProvider>().dropOverrideController.text.isNotEmpty){
                    context.read<JobsTodoProvider>().checkDropOverPass(context,widget.advertiseModel);
                  }
                  else{
                    if(context.read<JobsTodoProvider>().selected2.where((element) => element==false).isNotEmpty){
                      ...
                      context.read<JobsTodoProvider>().completeDropOff(widget.advertiseModel,context);
                    }
                  }
